...
# A matrix is a extended version of vectors (because, implicit coercion takes place)
# rbind() - bind element by row wise
# cbind() - bind element by column wise
# rownames(matrix_variable) - naming the rows (it should be equal to no. of rows)
# colnames(matrix_variable) - naming the cols (it should be equal to no. of cols)


matrix1 <- matrix(1:6,nrow=2)
matrix1

# matrix with given row and byrow and column will be automatically considered with the length of the vector
matrix2 <- matrix(1:6,nrow=2,byrow=TRUE)
matrix2

# matrix with byrow
matrix3 <- matrix(1:6,nrow=3,ncol=2,byrow=T)
matrix3
...
# it can be the mixture of datatype, implicit coercion will perform and make it as a same datatype
matrix7 <- matrix(c("One",2,"three",4,"five",6),nrow=2,ncol=3,byrow=T)
matrix7
class(matrix7)
typeof(matrix7)
...
matrix10 <- matrix(1:3,nrow=3,ncol=2) # (3*2 > 3)
matrix10

# size of the matrix should not be less than the size of the vector
matrix11 <- matrix(1:6,nrow=3,ncol=1) # (3*1 < 6)
matrix11

# if none of the nrow and ncol given, it arrange it consider ncol=1 and nrow=length of vector
matrix12 <- matrix(1:3)
matrix12
